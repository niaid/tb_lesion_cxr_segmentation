import optuna
import monai
from segment_tb_cxr.training.train_tb_segment import trainModel,_configure_data_preprocess,calculate_validation_loss
...
def objective(trial,train_csv_path, val_csv_path, model_info, output_model_filename, key):  

    trial_model_info = {}    
    for key in model_info["range_variables"].keys():      
        if type(model_info[key][0]) == int :
            trial_model_info[key] = trial.suggest_int(key, model_info["range_variables"][key][0],model_info["range_variables"][key][1])
        elif type(model_info[key][0]) == str:
            trial_model_info[key] = trial.suggest_categorical(key,model_info["range_variables"][key])
        else:
            trial_model_info[key] = trial.suggest_float(key,model_info["range_variables"][key][0],model_info["range_variables"][key][1])
            

    print(trial_model_info)
    train_loader,val_loader = _configure_data_preprocess(train_csv_path, val_csv_path, trial_model_info)  
...
def main(argv=None):
    ...
    parser.add_argument(
        "study_name",
        type=str,
        help="Study name in pickle format"
    )
    ...
    objective_with_args = partial(objective, train_csv_path=args.train_input_csv_path, 
