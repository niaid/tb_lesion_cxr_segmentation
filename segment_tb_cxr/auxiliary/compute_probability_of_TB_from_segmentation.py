...
def main():
    ...
    parser.add_argument(
        "input_csv_path",
        help="Enter the csv files containing filenames with column names\
        (processed_Filename,pred_tb_probability_arr_file_path,pred_lung_probability_arr_file_path",
    )
    parser.add_argument(
        "--threshold",
        default=0.5,
    ...
    parser.add_argument(
        "output_csv_file_path", help="Output CSV file path containing tb probabilities."
    )
    ...
    df[
        [
            "probabilty_of_tb_from_min_overall",
