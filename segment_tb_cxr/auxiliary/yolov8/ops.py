...
def xywhr2xyxyxyxy(rboxes):
    ...
    ctr = rboxes[..., :2]
    w, h, angle = (rboxes[..., i : i + 1] for i in range(2, 5))  # noqa: E203
...
def process_mask(
    protos, masks_in, bboxes, shape, upsample=False, additional_results=None
):
    """
    ...
    prob_masks = masks.clone()
    binary_masks = (masks > 0.5).float()

    if additional_results is not None:
        additional_results["prob_masks"] = prob_masks
