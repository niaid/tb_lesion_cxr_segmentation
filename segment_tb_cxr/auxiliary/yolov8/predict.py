...
class SegmentationPredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for i, pred in enumerate(p):
            ...
            additional_results = {"prob_masks": None, "non_cropped_masks": None}

            if not len(pred):  # save empty boxes
                masks = None  # noqa: F841
                binary_masks = None
                prob_masks = None  # noqa: F841
            elif self.args.retina_masks:
                pred[:, :4] = ops.scale_boxes(
                    img.shape[2:], pred[:, :4], orig_img.shape
                )
                binary_masks = ops.process_mask_native(
                    proto[i], pred[:, 6:], pred[:, :4], orig_img.shape[:2]
                )  # HWC
            else:

                binary_masks = ops.process_mask(
                    proto[i],
                    pred[:, 6:],
                    pred[:, :4],
                    img.shape[2:],
            ...
            results.append(
                Results(
                    ...
                    boxes=pred[:, :6],
                    masks=binary_masks,
                    prob_masks=additional_results["prob_masks"],
                )
            )
        return results
