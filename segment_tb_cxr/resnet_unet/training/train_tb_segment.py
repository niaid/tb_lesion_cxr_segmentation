...
import pathlib
import json
from segment_tb_cxr.resnet_unet.training.resnet18 import ResNetUNet
from torch.utils.tensorboard import SummaryWriter

"""
This script is used to train the TB segmentation model in Chest X Rays
that has 64%(approx.) of each dataset and valids on combined files that 16%(approx.)
of each dataset.
"""
...
def custom_train_collate_batch(batch):
    ...
    batch_data["img"] = torch.stack(
        [torch.tensor(batch[i][j]["img"]) for i in range(len(batch)) for j in range(len(batch[i]))]
    )
    batch_data["seg"] = torch.stack(
        [torch.tensor(batch[i][j]["seg"]) for i in range(len(batch)) for j in range(len(batch[i]))]
    )
...
def getTransforms(model_info):
   train_transforms = Compose(
       [
           LoadImaged(keys=["img", "seg"]),
           EnsureChannelFirstd(keys=["img", "seg"]),
           Resized(
               keys=["img", "seg"], spatial_size=model_info["img_size"], mode=("bilinear", "nearest")
           ),
           ...
           RandRotated(keys=["img", "seg"], range_x = (np.deg2rad(-model_info["range_variables"]["rotation_degree"]),np.deg2rad(model_info["range_variables"]["rotation_degree"])), prob=model_info["range_variables"]["prob_rotation"],mode=("bilinear", "nearest")),
...
def _configure_data_preprocess(
    train_csv_path, val_csv_path, model_info
):
    """
    ...
    Args:
        ...
        train_csv_path(list): Input CSV path containing training files with
                              column names as 'Filename' and 'Output_tb_seg_filename' representing
                              CXR and the corresponding binary TB mask respectively.
        val_csv_path(list):Input CSV path containing val files with
                              column names as 'Filename' and 'Output_tb_seg_filename' representing
                              CXR and the corresponding binary TB mask respectively.
        model_info(dict):W Dictionary containing information regarding training the model.
    Returns:
    ...
    val_files = [
        ...
        for cxr_file, ref_file in zip(val_files["processed_Filename"], val_files["Output_tb_seg_filename"])
    ]
...
def trainModel(train_loader, val_loader, model_info, output_model_filename):
    """
    ...
    if model_info["range_variables"]["optimizer"] == "Adam":
        optimizer = torch.optim.Adam(model.parameters(), lr=model_info["range_variables"]["learning_rate"])
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=model_info["range_variables"]["learning_rate"],momentum=model_info["range_variables"]["momentum"])
    ...
    for epoch in range(model_info["range_variables"]["epochs"]):
        ...
        if (epoch + 1) % val_interval == 0:
                val_images,val_labels, val_outputs, val_epoch_loss =  calculate_validation_loss(model,model_info,loss_function,val_loader,post_trans)
                writer.add_scalars(
                    "Loss",
    ...
    print(
        f"train completed, best_val_loss: {best_val_loss:.4f} at epoch: {best_val_loss_epoch}"
    )
    writer.close()
    

def main(argv=None):
    parser = argparse.ArgumentParser(description="TB Segmentation Model in Chest X Rays.")
    parser.add_argument(
